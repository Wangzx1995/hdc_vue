    ...
    <div class="h-perpetual-calendar">
        <div class="year-month-operate-bar">
            ...
            >
                ...
                <span>
                    <!-- 返回本月 -->
                    {{ $t("deviceHealthCondition.returnThisMonth") }}
                </span>
            </div>
        </div>
                        ...
                        >
                            ...
                            <slot :item="item">
                                ...
                                <div class="status">
                                    ...
                                    <span
                                        v-ellipsis
                                        v-if="item && item.isHoliday"
                                    >
                                        {{ text }}
                                    </span>
                                    <span v-else class="el-icon-plus" />
                                </div>
                            </slot>
                        </div>
                    </td>
...
export default {
    ...
    computed: {
        ...
        weekdays() {
            ...
            //   };
            const weekDate = {
                1: this.$t("common.weekDate1"),
                2: this.$t("common.weekDate2"),
                3: this.$t("common.weekDate3"),
                4: this.$t("common.weekDate4"),
                5: this.$t("common.weekDate5"),
                6: this.$t("common.weekDate6"),
                0: this.$t("common.weekDate7"),
            };
            const res = [];
    ...
    methods: {
        ...
        initDateList(year = this.currentYear, month = this.currentMonth) {
            ...
            const currentMonthFirstDay =
                calendar.getFirstDateDay(year, month) || 7;
            ...
            while (index > 0) {
                // 最后一个参数-1代表是上个月
                this.dateList.push(
                    this.createCalendarItem(year, month, 1 - index, -1)
                );
                index--;
            }
        ...
        createCalendarItem(y, m, d, flag) {
            ...
            if (calendarItem.lunarDate.lDay === 1) {
                ...
                calendarItem.lunarDate.fmtStr =
                    calendarItem.lunarDate.IMonthCn +
                    calendarItem.lunarDate.IDayCn;
            ...
            calendarItem.isHoliday =
                this.currentHolidayList.has(nowItemDateFormatStr);
            // 判断多选时是否被选中
            calendarItem.checked =
                this.isMultiSelect &&
                this.selectedItems.includes(nowItemDateFormatStr);
        ...
        mousedownHandler(item) {
            ...
            this.selectedStartItem = calendar.formatDateToString(
                item.date,
                "-"
            );
        ...
        mouseupHandler(item) {
            ...
            this.$emit(
                "update:holidayList",
                Array.from(this.currentHolidayList)
            );
            // 抛出dateChange时间
        ...
        isDisabledDate(item) {
            return (
                ...
                (this.currentYear === 2100 && this.currentMonth === 11) ||
                (this.$attrs["picker-options"] &&
                    this.$attrs["picker-options"].disabledDate(
                        new Date(item.date)
                    ))
            );
        },
